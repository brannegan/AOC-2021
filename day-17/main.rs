...
fn possible_velocities(ta: TargetArea) -> HashSet<(i32, i32)> {
    let mut vel_xs: Vec<RangeInclusive<i32>> = (1..)
        .take_while(|n| *ta.x_range.start() > n * (n - 1) / 2)
        .filter_map(|n| {
            ...
            start_vel_x_range
                .is_empty()
                .not()
                .then(|| start_vel_x_range)
        })
        .collect();
    let vel_ys: Vec<RangeInclusive<i32>> = (1i32..)
        //TODO: find proper terminating condition
        .take_while(|n| (ta.y_range.end() - ta.y_range.start()) >= *n / 5)
        .filter_map(|n| {
